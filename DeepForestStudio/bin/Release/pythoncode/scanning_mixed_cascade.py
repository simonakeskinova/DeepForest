...
class ScanningMixedCascade(object):
    ...
    def __init__(self, config):
        ...
        self.max_levels_count = self.scanning_config["MaxLevelsCount"]
    ...
    def train(self, X, y): 
        ...
        while 1:        
            ...
            """if level_count == 0:
                ansamble = Ansamble(self.cascade_config, self.classes_count)
                y_proba = ansamble.train(self.X_wins1d[0], y, None, None)
            else:"""
            window_level = WindowLevel(self.cascade_config, len(self.X_wins1d), self.classes_count, self.zero_based_classes)
            y_proba = window_level.train(self.X_wins1d, y, y_proba)                
                        
            accuracy = calc_accuracy(y, y_proba, self.name, self.classes_count, "train_window_cascade", show_matrix=False)
        ...
        return self.optimal_level_count
    ...
    def predict_proba(self, X):
        ...
        for level in range(self.optimal_level_count):
            if self.keep_model_in_mem:
                classifier = self.levels[level]
            else:
                classifier = self.load_estimator(level)   
            
            """if level == 0:
                y_curr_proba = classifier.predict_proba(X_test_wins[0], None)
                #y_proba = y_curr_proba
            else:"""
            y_curr_proba = classifier.predict_proba(X_test_wins, y_curr_proba)
                #y_proba += y_curr_proba
